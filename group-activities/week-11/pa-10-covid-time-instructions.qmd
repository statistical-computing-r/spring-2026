...
1. completed [pa-10-covid-time-activity.qmd](../week-10/pa-9-covid-factors-activity.qmd)
...
cases |> 
  ...
  mutate(date = ___(date),
         ...
         year = ___(date),
         yr = str_replace(year, pattern = ,replacement = )) |> 
  unite()
```
...
```{r}
#| label: covid-graph-cali-solution
#| echo: false
...
ca_only_month |> 
  ...
  ggplot(aes(y = fct_reorder(month_year, desc(y_m)),
             x = total_cases, 
  ...
  labs(x = "Number of COVID-19 Cases in California (Log 10)",
       y = "Month and Year",
       fill = "Season",
