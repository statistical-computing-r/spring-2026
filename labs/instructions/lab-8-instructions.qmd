...
::: callout-tip
# You can use `across()` or look at variations on `summarize()`.  
:::
...
```{r}
#| label: find-missing-values-solution
#| echo: false
...
```{r}
#| label: map-missing-values-of-fish-solution
#| echo: false
...
```{r}
#| label: visual-of-missing-values-over-time-solution
#| echo: false
...
fish |>
  ...
  ggplot(aes(x = year, 
             ...
             color = factor(trip))) +
  geom_point() +
  geom_line(aes(linetype = section)) +
  labs(x = "Year", 
       y = "Missing weight values", 
       color = "Trip",
       linetype = "Section",
       title = "Missing values in Blackfoot Fish data") +
...
=======
  guides(fill = "none")
>>>>>>> 9560eed02d4a2631da3247a02d2dd18eabbf6a37
...
```{r}
#| label: write-rescale-function-solution
#| echo: false

rescale_01 <- function(x) {
  stopifnot("Non-numeric data provided" = is.numeric(x), 
            "Not enough data provided" = length(x) > 1)
  (x - min(x, na.rm = TRUE)) /
...
rescale_01 <- function(x) {
  if (length(x) <= 1) stop("length of vector not greater than 1")
...
```{r}
#| label: rescale-data-frame-function-solution
#| echo: false
